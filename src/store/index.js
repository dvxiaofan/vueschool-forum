...
export default createStore({
  ...
  actions: {
    ...
    fetchItem ({ commit, state }, { id, emoji, resource }) {
      ...
      return new Promise((resolve) => {
        firebase.firestore().collection(resource).doc(id).onSnapshot(snapshot => {
          const item = { ...snapshot.data(), id: snapshot.id }
          commit('setItem', { resource, item })
          resolve(item)
        })
        // setTimeout(() => {
        //   const item = findById(state[resource], id)
        //   resolve(item)
        // }, 1000)
      })
    },
...
function makeAppendChildToParentMutation ({ parent, child }) {
  return (state, { childId, parentId }) => {
    ...
    if (!resource) return console.warn(`Appending ${child} ${childId} to ${parent} ${parentId} failed because parent didn't exist`)
